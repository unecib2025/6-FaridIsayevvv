...
# Контрольные вопросы
'''
1. Append()/insert() отличаются тем, что первый добавляет элемент в конец списка, а второй в указанную позицию
2. Вырезает последний элемент списка и называет его
3. remove() за раз удаляет только один элемент
4. он меняет список делая его в обратном порядке
5. count() показывает количество указанного элемента, а index() - индекс указанного элемента
6. Изменяют список: append(), extend(), insert(), remove(), pop(), clear(), sort(), reverse()
   Возвращают новый список: count(), index()
7. Произойдет ошибка
8. Потому что она помогает упорядочить данные и делает их более читаемыми
'''